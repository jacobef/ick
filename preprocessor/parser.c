...
static void complete(struct earley_rule *const rule, erule_p_vec *const out) {
    ...
    for (size_t i = 0; i < rule->origin_chart->n_elements; i++) {
        const struct earley_rule possible_origin = *rule->origin_chart->arr[i];
        if (!possible_origin.dot->is_terminal && !is_completed(possible_origin) && possible_origin.dot->val.rule == rule->lhs) {
            erule_p_vec new_completed_from = erule_p_vec_new(possible_origin.completed_from.n_elements + 1);
...
static erule_p_vec *next_chart(const erule_p_vec *const old_chart, const struct preprocessing_token token) {
    ...
    *out = erule_p_vec_new(0);
    ...
    for (size_t i = 0; i < out->n_elements; i++) {
        struct earley_rule *const rule = out->arr[i];
        complete(rule, out);
...
void print_chart(const erule_p_vec *const chart) {
    for (size_t i = 0; i < chart->n_elements; i++) {
        const struct earley_rule rule = *chart->arr[i];
        print_rule(rule);
        printf("\n");
    }
}
...
erule_p_vec_p_vec make_charts(const pp_token_vec tokens, const struct production_rule *const start_rule) {
    ...
    *initial_chart = erule_p_vec_new(1);
    ...
    for (size_t i = 0; i < initial_chart->n_elements; i++) {
        struct earley_rule *const rule = initial_chart->arr[i];
    ...
    for (size_t i = 0; i < tokens.n_elements; i++) {
        ...
        clear_color();
        print_with_color(TEXT_COLOR_RED, "):\n");
        erule_p_vec *const new_chart = next_chart(old_chart, tokens.arr[i]);
...
static struct earley_rule *get_tree_root(const erule_p_vec_p_vec charts, const struct production_rule *const root_rule) {
    const erule_p_vec *const final_chart = charts.arr[charts.n_elements-1];
    for (size_t i = 0; i < final_chart->n_elements; i++) {
        struct earley_rule *const rule = final_chart->arr[i];
...
static pp_token_vec get_text_line_tokens(const struct earley_rule text_line_rule) {
    ...
    const struct earley_rule *const tokens_not_starting_with_hashtag_opt_rule = text_line_rule.completed_from.arr[0];
    if (tokens_not_starting_with_hashtag_opt_rule->rhs.tag == OPT_NONE) {
        return text_line_tokens;
    }
    const struct earley_rule *const tokens_not_starting_with_hashtag_rule = tokens_not_starting_with_hashtag_opt_rule->completed_from.arr[0];

    for (size_t i = 0; i < tokens_not_starting_with_hashtag_rule->completed_from.n_elements; i++) {
        const struct earley_rule *const non_hashtag_rule = tokens_not_starting_with_hashtag_rule->completed_from.arr[i];
...
static void deal_with_macros(const struct earley_rule root) {
    const struct earley_rule group_opt_rule = *root.completed_from.arr[0];
    if (group_opt_rule.rhs.tag == OPT_NONE) {
        printf("no groups\n");
        return;
    }
    const struct earley_rule group_rule = *group_opt_rule.completed_from.arr[0];
    ...
    for (size_t i = 0; i < group_rule.completed_from.n_elements; i++) {
        const struct earley_rule group_part_rule = *group_rule.completed_from.arr[i];
        switch ((enum group_part_tag)group_part_rule.rhs.tag) {
            case GROUP_PART_CONTROL: {
                const struct earley_rule control_line_rule = *group_part_rule.completed_from.arr[0];
            ...
            case GROUP_PART_TEXT: {
                const struct earley_rule text_line_rule = *group_part_rule.completed_from.arr[0];
            ...
            case GROUP_PART_IF: {
                const struct earley_rule if_section_rule = *group_part_rule.completed_from.arr[0];
